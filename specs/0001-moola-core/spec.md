...
- REQ-025: On a migrated installation with no users, normal browser requests must redirect to a simple first-run setup screen that creates the initial household and administrator. This one-time setup must require the administrator's name, email address, and password, must never use default credentials, and must become inaccessible as soon as the first user exists. This bootstrap flow is distinct from public registration, which remains unavailable.
...
- AC-022: Given a migrated database with no users, opening the application redirects to `/setup`; submitting valid household and administrator details creates exactly one Argon2id-protected administrator, signs that user in, and opens the application. Afterward, both direct GET and POST access to setup cannot create another administrator, while `/register` remains unavailable. (REQ-015, REQ-019, REQ-020, REQ-025)
